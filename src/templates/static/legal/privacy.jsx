...
const Privacy = () => (
    <div>
        ...
        <p>{it.L('The Company\'s cookies are not deemed dangerous and cannot access any other information on your computer.')}</p>
        <p>{it.L('By using or interacting with the [_1] website, the client is giving permission to the use of the Google Analytics User ID Feature, which allows Google to capture the client\'s [_1] login ID such as VRTC1234 and MT1234. When this feature is enabled, no other personally identifiable information other than the client\'s [_1] login ID, or other data that can be linked to such information by Google, is shared with or disclosed to Google.', it.website_name)}</p>
        ...
        <ol>
            ...
            <li>{it.L('The client is responsible for keeping the client\'s log-in details secure, the security of any linked email address, and the security (password protection, screen locking, etc) of any personal computer or device on which the client\'s account is accessible. The Company shall not be held responsible if there is unauthorised use of the account when the Company is not at fault.')}</li>
        </ol>
        <br />
        ...
        <div data-show='eucountry'>
            ...
            <p>{it.L('The Company\'s data protection officer can be contacted at [_1]', 'dpo@binary.com')}</p>
