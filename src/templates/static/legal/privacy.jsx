...
const JobApplicantPrivacyPolicy = () =>  (
    <React.Fragment>
        ...
        <p>{it.L('The Company collects, stores, and processes a range of information about the applicant, such as, but not limited to:')}</p>
        <ul className='bullet'>
            <li>{it.L('the applicant\'s personal details, including name, address, and contract details, comprising email address and telephone number, date of birth, and gender;')}</li>
            <li>{it.L('details of the applicant\'s about the applicant\'s remuneration, including entitlement to benefits such as pensions or insurance cover, if applicable;')}</li>
            <li>{it.L('information about the applicant\'s nationality;')}</li>
            <li>{it.L('details of the identity of the the applicant\'s reference, including their name and contract details, comprising email address and telephone number; and')}</li>
            <li>{it.L('information about medical or health conditions, including whether or not the applicant has a disability for which the Company needs to make reasonable adjustments.')}</li>
        </ul>
        <p>{it.L('The Company may collect this information in a variety of ways. Data is collected and contained in CVs or resumes, via Self-Assessment Topgrading Questionnaires/Interview (SATI), or through interviews or other forms of assessment, including online tests.')}</p>
        <p>{it.L('The Company may also collect personal data about the applicant from third parties, such as further references supplied by former employers and information from criminal records checks. ')}</p>
        ...
        <p>{it.L('The Company typically collects and uses your the applicant\'s Personal Data for the purposes of the legitimate interests pursued by the Binary Group, such as making decisions about the person to employ and the salary and other benefits to offer; and complying with our the Company\'s legal obligations, such as conducting necessary checks in relation to the right to work in the a specific jurisdiction and making any reasonable adjustments for disabled employees. ')}</p>
        <p>{it.L('The applicant is under no statutory or contractual obligation to provide data to Binary Group during the recruitment process. However, should the applicant elect not to provide any necessary information, the Company may not be able to proceed with their application.')}</p>
        ...
        <p>{it.L('The Company shall further disclose information about the applicant to any third-party parties\' service providers within the Binary Group. The Company shall ensure that a data sharing agreement is in place and that the applicant\'s data is held securely and in line with GDPR requirements. The Company shall further disclose information about the applicant when it is legally obligated to do so.')}</p>

        <h2 data-anchor='data-retention'>{it.L('Data retention')}</h2>
        <p>{it.L('The applicant\'s personal data shall be stored for a period of six months should the applicant be unsuccessful or decline in accepting employment with the Binary Group. At the expiry of that period, the applicant\'s data is deleted or destroyed unless the Company is required to further retain your the applicant\'s information to exercise or defend any legal claims.')}</p>
        <p>{it.L('Should your the applicant\'s application be successful and an employment with the Binary Group be accepted, the applicant\'s data will be retained until the cessation of their employment. A separate privacy policy for employees will be made available to the applicant after entering employment with the Company.')}</p>

        <h2 data-anchor='applicants-rights'>{it.L('Applicant\'s rights')}</h2>
        <p>{it.L('In light of the EU General Data Protection Regulation 2016/679 and the Data Protection Act 2018, the applicant is entitled to a number of rights with regards to the applicant\'s personal data; namely, the following rights:')}</p>
        <ul className='bullet'>
            <li>{it.L('The right to be informed of the use of their personal data. This privacy policy is dedicated for this purpose.')}</li>
            <li>{it.L('The right to request access to their data from the Company.')}</li>
            <li>{it.L('The right to rectify their personal data should the Company hold any information about the applicant that is incomplete or inaccurate.')}</li>
            <li>{it.L('The right to erase or delete their personal information subject to the data retention practice of the Binary Group.')}</li>
        </ul>

        <p>{it.L('The applicant is further afforded the rights to:')}</p>
        <ul className='bullet'>
            <li>{it.L('restrict the further processing of their personal data, as well as in certain circumstances; and')}</li>
            <li>{it.L('the right to data portability')}</li>
        </ul>
