...
const BIEL = () => (
    <div data-show='eucountry'>
        ...
        <p>{it.L('Binary Investments (Europe) Ltd aggregates the Bid and Ask prices from a pool of liquidity providers to determine and offer the client the best available price of the instrument. The Company is always the final counterparty to the client\'s Transactions. The Company may execute Orders on behalf of its client, wherein the Company will act as a broker and pass on the Order to the liquidity provider for execution. Alternatively, the Company may also keep the client\'s Order on its own account, wherein the Company will take the other side of the client\'s trade. It is further noted that the Company may make a profit as a result of the client\'s losses and vice-versa. Further details can be found under Conflicts of interest policy and Order execution policy.')}</p>
