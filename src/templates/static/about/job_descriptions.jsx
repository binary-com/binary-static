...
[% BLOCK column %]
    <div className='eq'>
        <h3[% IF class %] className='[% class %]'[% END %]>[% header %]</h3>
        <p>[% text %]</p>
        <a href={it.url_for(\'/open-positions/job-details?dept=\' _ id _ \'#\' _ subsection)}>{it.L('Learn more')}</a>
    </div>
[% END %]
    ...
    <div className='gr-row gr-padding-10' id='[% id %]'>
        <div className='gr-3 gr-12-m'>
            <div className='gr-12 gr-padding-10 center-text'>
                ...
                <img className='responsive' src={it.url_for('images/pages/careers/\' _ id _ \'.svg')}/>
            </div>
        </div>
        <div className='gr-9 gr-12-m'>
            <div className='eqWrap'>
                [% FOREACH section IN sections %]
                    [% INCLUDE column header=section.header text=section.text subsection=section.subsection className=section.class id %]
                [% END %]
            </div>
        </div>
    </div>
...
<div className='open-positions container'>
    <div className='gr-parent static_full'>
        ...
        [% INCLUDE row id='information_technology' title=l('Information Technology') sections=[
            ...
            { header=l('Senior Perl Developer'),      subsection=\'senior_perl_developer\',     text=l('Tasked with writing robust, high-quality, production-ready code and contributing to the architecture that drives our high-traffic global website.'), className='one-line nowrap' },
        ] %]

        [% INCLUDE row id='quality_assurance' title=l('Quality Assurance') sections=[
            { header=l('Quality Assurance Engineer'), subsection=\'quality_assurance_engineer\', text=l('Execute and update manual and automated test plans, ensure the integrity of releases, investigate and reproduce client-reported issues.'), className='nowrap' },
        ] %]
        ...
        [% INCLUDE row id='quantitative_analysis' title=l('Quantitative Analysis') sections=[
            ...
            { header=l('Quantitative Analyst'),   subsection=\'quantitative_analyst\',   text=l('Responsible for the pricing of binary options offered on the [_1] website and the risk management and profitability of its options book.', it.website_name) },
        ] %]
