...
const BeginnersFaq = () => (
    <div className='section-content section-10'>
        ...
        <p>{it.L('The first step is to open an account. You can <a target=\'_blank\' href=\'[_1]\'>apply online</a> in just a few minutes.', request.url_for('/'))}</p>
        ...
        <p>{it.L('[_1] has thousands of clients taking a variety of positions on the financial markets at any one time, and earns a small margin on these trades.', website_name)}</p>
        ...
        <p>{it.L('[_1] offers a range of common deposit and withdrawal methods, from credit and debit cards to bank wires, e-cash and e-wallets.', website_name)}</p>
        ...
        <p>{it.L('[_1] offers a virtual-money account so you can get the hang of trading binaries. Why not <a href=\'[_2]\'>open a free \$10k fully-functional virtual-money account</a>, and start learning how to trade.', website_name, request.url_for('/'))}</p>

        <h3>{it.L('Will I need to install any software?')}</h3>
        <p>{it.L('[_1] is entirely web-based and requires no software installation.', website_name)}</p>

        <h3>{it.L('How soon can I start trading?')}</h3>
        <p>{it.L('You can <a href=\'[_1]\'>open an account</a>, deposit funds and begin trading within minutes.', request.url_for('/'))}</p>
    </div>
);
