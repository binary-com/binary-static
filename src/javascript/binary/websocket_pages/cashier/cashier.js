...
const Cashier = (function() {
    ...
    const check_locked = function() {
        if (Client.get_value('is_virtual')) return;
    ...
    const check_top_up_withdraw = function() {
        if (is_cashier_page() && Client.get_value('values_set')) {
    ...
    const onLoad = function() {
        if (is_cashier_page() && Client.get_value('is_logged_in')) {
    ...
    const onLoadPaymentMethods = function() {
        ...
        if (Client.get_value('is_logged_in') && !Client.get_value('is_virtual')) {
            Cashier.check_locked();
        }
    };
