...
const Cashier = (() => {
    ...
    const displayTopUpButton = (currency, balance) => {
    ...
    const onLoad = () => {
        ...
        if (Client.isLoggedIn()) {
            BinarySocket.wait('authorize', 'balance').then(() => {
                ...
                const balance_response = State.get(['response', 'balance', 'balance']) || {};
                const balance          = +balance_response.balance;
                const is_virtual = Client.get('is_virtual');
                const is_crypto  = isCryptocurrency(balance_response.currency);
                if (is_virtual) {
                    displayTopUpButton(balance_response.currency, balance);
                }
