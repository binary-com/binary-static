  ...
  // function to set an orange circle on the entry tick
    ...
    function draw_barrier() {
        if (chart.yAxis[0].plotLinesAndBands.length === 0) {
            if (contract.barrier) {
                chart.addPlotLineY({ id: 'barrier', value: contract.barrier * 1, label: localize('Barrier ([_1])').replace('[_1]', contract.barrier), dashStyle: 'Dot' });
            } else if (contract.high_barrier && contract.low_barrier) {
                chart.addPlotLineY({ id: 'high_barrier', value: contract.high_barrier * 1, label: localize('High Barrier ([_1])').replace('[_1]', contract.high_barrier), dashStyle: 'Dot' });
                chart.addPlotLineY({ id: 'low_barrier', value: contract.low_barrier * 1, label: localize('Low Barrier ([_1])').replace('[_1]', contract.low_barrier), dashStyle: 'Dot' });
            }
        }
    }
