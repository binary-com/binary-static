var Highchart = (function() {
  var chart, options, chart_forget, responseID, contract, contract_ended, contracts_for_send, history_send, entry_tick_barrier_drawn, initialized, chart_delayed, chart_subscribed, request, min_point, max_point, start_time, purchase_time, now_time, end_time, entry_tick_time, is_sold, sell_time, sell_spot_time, is_expired, is_after_settlement, exit_tick_time, exit_time;
  function init_once() {
      chart = '';
      initialized = false;
      chart_delayed = false;
  ...
  var dispatch = function(response) {
    ...
    if (type === 'contracts_for' && (!error || (error && error.code && error.code === 'InvalidSymbol'))) {
      ...
      if (response.history || response.candles) {
        ...
        if (is_sold || (is_expired && is_after_settlement)) {
          reset_max();
          reselect_exit_time();
          end_contract();
        }
  ...
  function show_chart(proposal_contract, update) {
      ...
      if ((is_sold || (is_expired && is_after_settlement)) && chart) {
        reset_max();
        reselect_exit_time();
        end_contract();
      }
  ...
  function get_max_history(response) {
    ...
    if (response.history && response.history.times && ((is_expired && is_after_settlement) || is_sold)) {
  ...
  function get_max_candle(response) {
    ...
    if ((is_expired && is_after_settlement) || is_sold) {
  ...
  function forget_streams() {
    if (chart && chart.series && chart.series[0].data.length >= 1 && !chart_forget && (is_sold || (is_expired && is_after_settlement)) && responseID) {
