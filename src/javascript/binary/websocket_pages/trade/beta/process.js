...
function processContract_Beta(contracts) {
    ...
    if (contracts.hasOwnProperty('error') && contracts.error.code === 'InvalidSymbol') {
        ...
        elementInnerHtml(confirmation_error_contents, contracts.error.message + ' <a href="javascript:;" onclick="sessionStorage.removeItem(\'underlying\'); window.local_location.reload();">' + localize('Please reload the page') + '</a>');
        return;
    }
