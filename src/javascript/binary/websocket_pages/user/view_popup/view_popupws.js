...
var ViewPopupWS = (function() {
    ...
    var normalUpdate = function() {
        ...
        if (contract.barrier_count > 1) {
            ...
            containerSetText('trade_details_barrier',     contract.entry_tick_time ?
                (contract.contract_type === 'DIGITMATCH' ? text.localize('Equals') + ' ' + contract.barrier :
                    contract.contract_type === 'DIGITDIFF' ? text.localize('Not') + ' ' + contract.barrier :
                    contract.barrier) : '-',
                '', true);
        }
    ...
    var normalRow = function(label, label_id, value_id, isHidden, value) {
        return '<tr' + (isHidden ? ' class="' + hiddenClass + '"' : '') + '><td' + (label_id ? ' id="' + label_id + '"' : '') + '>' + text.localize(label) + '</td><td' + (value_id ? ' id="' + value_id + '"' : '') + '>' + (value || '') + '</td></tr>';
    };
