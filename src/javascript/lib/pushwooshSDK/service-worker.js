...
var PUSHWOOSH_DEFAULT_IMAGE      = 'https://cp.pushwoosh.com/img/logo-medium.png'; // TODO: change to bull/bear logo from style.binary.com
var PUSHWOOSH_DEFAULT_TITLE      = 'Binary.com';
...
var PushwooshBase = function() {
  this.APPLICATION_CODE = 'D04E6-FA474'; // Your Application Code from Pushwoosh
...
PushwooshBase.prototype.doPushwooshApiMethod = function(methodName, arguments) {
  if (DEBUG_MODE) console.log('Performing %s call to Pushwoosh with arguments: %s', methodName, JSON.stringify(arguments));
  try {
...
PWServiceWorker.prototype.pushReceived = function(event) {
  ...
  event.waitUntil(
    self.registration.pushManager.getSubscription().then(thisO.fetchLastPushMessage).then(thisO.createPushFromResponse).catch(function(err) {
