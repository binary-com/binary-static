    ...
    const withdrawResponse = (response) => {
        ...
        switch (response.paymentagent_withdraw) {
            case 2: { // dry_run success: showing the confirmation page
                ...
                $('#lblAmount').text(getTextFormat(request.amount, request.currency));

                FormManager.init(view_ids.confirm, [
                    { request_field: 'paymentagent_loginid',  value: request.paymentagent_loginid },
                    { request_field: 'amount',                value: getTextFormat(request.amount, request.currency) },
            ...
            case 1: // withdrawal success
                ...
                $('#successMessage').css('display', '')
                    ...
                    .html($('<ul/>', { class: 'checked' }).append($('<li/>', { text: localize('Your request to withdraw [_1] [_2] from your account [_3] to Payment Agent [_4] account has been successfully processed.', [request.currency, getTextFormat(request.amount, request.currency), Client.get('loginid'), agent_name]) })));
                break;
