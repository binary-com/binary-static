...
const MetaTraderConfig = (() => {
    ...
    // or just 1 if both accounts have the same currency
    const getMinMT5TransferValue = (binary_to_mt5 = true) => {
        const binary_currency = Client.get('currency');
        const mt5_currency    = getCurrency(Client.get('mt5_account'));
        if (binary_currency === mt5_currency) return 1;
        const currency = binary_to_mt5 ? binary_currency : mt5_currency;
    ...
    const getMaxMT5TransferValue = (binary_to_mt5 = true) => {
        const binary_currency = Client.get('currency');
        const mt5_currency    = getCurrency(Client.get('mt5_account'));
        const currency        = binary_to_mt5 ? binary_currency : mt5_currency;
    ...
    const validations = () => ({
        ...
        deposit: [
            { selector: fields.deposit.txt_amount.id, validations: [['req', { hide_asterisk: true }], ['number', { type: 'float', min: () => getMinMT5TransferValue(), max: () => Math.min(State.getResponse('get_limits.remainder') || getMaxMT5TransferValue(), getMaxMT5TransferValue()).toFixed(Currency.getDecimalPlaces(Client.get('currency'))), decimals: Currency.getDecimalPlaces(Client.get('currency')) }], ['custom', { func: () => (Client.get('balance') && (+Client.get('balance') >= +$(fields.deposit.txt_amount.id).val())), message: localize('You have insufficient funds in your Binary account, please <a href="[_1]">add funds</a>.', [urlFor('cashier')]) }]] },
        ],
        withdrawal: [
            { selector: fields.withdrawal.txt_main_pass.id, validations: [['req', { hide_asterisk: true }]] },
            { selector: fields.withdrawal.txt_amount.id,    validations: [['req', { hide_asterisk: true }], ['number', { type: 'float', min: () => getMinMT5TransferValue(false), max: () => getMaxMT5TransferValue(false), decimals: 2 }]] },
        ],
    });
