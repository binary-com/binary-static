    ...
    const displayAccountStatus = () => {
        BinarySocket.wait('get_account_status', 'authorize', 'landing_company').then(() => {
            ...
            const messages = {
                ...
                expired              : () => buildSpecificMessage(localizeKeepPlaceholders('Your [_1]proof of identity[_3] and [_2]proof of address[_3] have passed their expiration date.'),                              [`<a href='${Url.urlFor('user/authenticate')}'>`, `<a href='${Url.urlFor('user/authenticate')}?authentication_tab=poa'>`, '</a>']),
                expired_identity     : () => buildMessage(localizeKeepPlaceholders('Your [_1]proof of identity[_2] has passed expiration date.'),                                                                    'user/authenticate'),
                expired_document     : () => buildMessage(localizeKeepPlaceholders('Your [_1]proof of address[_2] has passed expiration date.'),                                                                     'user/authenticate', '?authentication_tab=poa'),
