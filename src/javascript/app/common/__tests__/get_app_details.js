...
describe('GetAppDetails', () => {
    ...
    describe('.addAppIdName()', () => {
        it('works as expected', () => {
            expect(GetAppDetails.addAppIdName(2, 'Binary.com')).to.equal('data-balloon="Transaction performed by Binary.com (App ID: 2)"');
        });
        it('doesn\'t show for same app id as current', () => {
            expect(GetAppDetails.addAppIdName(1, 'Binary.com')).to.equal('');
        });
    });
});
