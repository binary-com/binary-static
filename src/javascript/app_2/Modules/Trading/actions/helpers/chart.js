...
export const createChartBarriersConfig = (store, proposal_response) => {
    ...
    // });
    //
    // const config = new ChartBarrierConfig(barrier, ...);
    // return { main: config };
};

export const updateBarrierShade = (store, is_over, contract_type) => {
    const config = store.chart_barriers.main; // get(store.chart_barriers, 'main');
    ...
    return shade;
    //return { main: Object.assign(config, { shade }) };
};
